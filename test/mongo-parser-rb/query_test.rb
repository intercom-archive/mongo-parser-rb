...
class QueryTest < MiniTest::Unit::TestCase
 ...
 def test_integer_gte
  ...
  def test_integer_lte_and_integer_gt
    ...
    assert !query.matches_document?(:integer_key => 9, :integer_key_2 => 4)
    assert !query.matches_document?(:integer_key => 11, :integer_key_2 => 4)
  end

  def test_string_eq
    query = MongoParserRB::Query.parse(:string_key => "hello world")
    assert query.matches_document?(:string_key => "hello world")
    assert !query.matches_document?(:string_key => 1)
    assert !query.matches_document?(:string_key => "bye world")
  end
  ...
  def test_string_lt
    ...
    assert query.matches_document?(:string_key => "abc")
    assert !query.matches_document?(:string_key => "xyz")
  end
  ...
  def test_string_and_integer_equality
    ...
    assert !query.matches_document?(:string_key => "xyz", :integer_key => 5)
    assert !query.matches_document?(:string_key => "abc", :integer_key => 4)
  end
  ...
  def test_or
    ...
    assert query.matches_document?(:string_key => "abc")
    assert !query.matches_document?(:string_key => "cde")
  ...
  def test_query_field_integration
    ...
    assert !query.matches_document?(:custom_data => {:tracked_users => 1})
  end
  ...
  def test_operator_data_type_mismatch
    query = MongoParserRB::Query.parse(:array_key => {:$in => [1]})
    assert !query.matches_document?(:array_key => "hey")
  end
  ...
  def test_date_range
    ...
    assert !query.matches_document?(:date_key => Time.new(1992,2,13,0,0,0))

    query = MongoParserRB::Query.parse(:date_key => {:$gt => Time.new(1993,2,13,0,0,0), :$lt => Time.new(1995,2,13,0,0,0)})
    assert query.matches_document?(:date_key => Time.new(1994,2,13,0,0,0))
    assert !query.matches_document?(:date_key => Time.new(1996,2,13,0,0,0))
  end
